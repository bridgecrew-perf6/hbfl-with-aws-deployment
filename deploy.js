...
function unpack (ip) {
  return new Promise((resolve, reject) => {
    const command = 
      'sudo unzip -o -q ./archive.zip -d hbfl && ' +
      'cd hbfl && ' +
      'sudo npm i && ' + // installs dependencies
      //'kill -9 $(pgrep -f node) && ' + // this kills any existing node processes except self
      'sudo npm start' // starts app
